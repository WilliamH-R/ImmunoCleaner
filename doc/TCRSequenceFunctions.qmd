...
By inputting a tidy data frame, a scatter plot is provided as output which shows number of relevant bindings for each allele. It is done by, for each barcode, making pMHC distinct, ungrouping and simply count the alleles. The output is then as shown in @fig-count_binding_pr_allele.

```{r}
#| label: fig-count_binding_pr_allele
#| fig-cap: "Output of the function `count_binding_pr_allele()`. The y-axis shows the count of relevant binders for each label (which is the name of the allele)."

data_donor_four_tidy %>%
  count_binding_pr_allele()
```

## `predict_allele_typing()` {#sec-predict_allele_typing}

For each of the donors who participated in the experiment, the HLA typing was noted @10XGenomics2022. In some instances, the allele typing is missing. The purpose of this function is to predict the missing allele typing. The top two scoring HLA's (i.e., HLA's with the highest number of relevant binders) are found from the input tidy data frame. Then, depending on the input file, either the highest or second highest scoring HLA is returned. If the top scoring HLA is already noted as a typing, then second highest scoring is instead returned. Further, if both HLA typings are already present a string will be returned informing, that both typings are already present. The output is simply a string as shown in @lst-predict_allele_typing.

``` {#lst-predict_allele_typing .R lst-cap="Output of the function predict_allele_typing(). If donor which the input data frame refers to are missing a HLA type, a prediction of the typing is returned. Otherwise a string will be returned informing, that both HLA typings are present."}

data_donor_one_tidy %>%
  predict_allele_typing()

[1] "The predicted allele typing is: B0801"

data_donor_four_tidy %>%
  predict_allele_typing()
  
[1] "You chose a data set where all HLA typings are known"
```
