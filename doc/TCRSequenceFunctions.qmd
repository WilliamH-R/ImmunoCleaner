...
: Shows how the TCR-sequences are noted in the tidy data file. Now, each sequence is on its own row and the chain indicator has been moved to a new column `chain`. If cell only contained one alpha and one beta sequence in the raw data file, that sequence will appear in the new column `non_promiscuous_pair`. {#tbl-TCR_sequence_example_after}
...
4.  Disregard a entire barcode if it shows to be relevant towards more than four pMHC (10x Genomics standard)
