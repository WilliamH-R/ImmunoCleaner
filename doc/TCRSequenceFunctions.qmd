...
With all the advancements in recent years in regards to computational power, laboratory techniques and -equipment, the amount of produced data is ever increasing. The focus in this article will be on biological data, even though it applies to a range of fields. For a reference on how quick the situation develops, the cost of sequencing the entire human genome decreased from (INSERT VALUE) to (INSERT VALUE) in a matter of (INSERT VALUE). The development obviously didn't stop there. As a more recent example, (INSERT EXAMPLE).
