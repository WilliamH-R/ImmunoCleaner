...
#'
#' @return The same data frame, but with an added column `TCR_sequence_pair`.
  ...
  data_aug <-
    ...
    dplyr::left_join({{file_name}} %>%
                       dplyr::select(barcode,
                                     cell_clono_cdr3_aa),
                     by = "barcode") %>%
    dplyr::mutate(TCR_sequence_pair = dplyr::case_when(stringr::str_count(string = cell_clono_cdr3_aa,
                                                                          pattern = "TRA") == 1 &
